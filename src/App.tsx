  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(cardRef.current, {
        scale: 1, // Minimum scale for maximum compatibility
        ...
        onclone: (clonedDoc) => {
          ...
          if (card) {
            ...
            card.style.borderRadius = '0'; // Simplify for canvas
            card.style.boxShadow = 'none';
            
            // Remove the background image entirely for the capture if it's causing issues
            const bg = card.querySelector('div[style*="tech-bg.png"]');
            if (bg instanceof HTMLElement) {
              bg.style.display = 'none';
              card.style.backgroundColor = '#050505';
            }

            // Remove all blurs and filters manually
      ...
      link.download = `GIMT_Pass_${data.name.replace(/\s+/g, '_') || 'Guest'}.png`;
      link.href = dataUrl;
      ...
      alert('Your browser is preventing the automated download. Please take a quick screenshot to save your pass! It looks amazing!');
    }
  };
